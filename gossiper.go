...
import (
	...
	"github.com/maxpoletaev/gossip/internal/rolling"
	"github.com/maxpoletaev/gossip/proto"
	"github.com/maxpoletaev/gossip/transport"
)
...
func (g *Gossiper) processRepeatReq(msg *proto.GossipMessage, req *proto.RepeatReq) {
	...
	if len(missingIDs) > 0 && msg.Ttl > 0 {
		seqNums := make([]uint64, 0, len(missingIDs))
		for seqNum := range missingIDs {
...
func (g *Gossiper) gossip(msg *proto.GossipMessage) error {
	...
	peerIDs := make([]PeerID, 0, len(knownPeers))
